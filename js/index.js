document.addEventListener("DOMContentLoaded", function () {
    ...
    function controlarPeca(pecaId, pecaCenarioId) {
        ...
        if (pecaParaAnimar === pecaId && peca) {
            // MOSTRAR A PEÇA ANTES DE INICIAR A ANIMAÇÃO
            peca.classList.add('mostrar-peca');
            ...
            peca.addEventListener('animationend', function() {
                ...
                // Marcar como animada permanentemente
                peca.classList.remove('mostrar-peca');
                peca.classList.add('peca-animada');
                
                // Verificar se o jogo está completo após cada animação
                checkIfGameComplete();
            });
        } else if (sessionStorage.getItem(`${pecaId}Animada`) && peca) {
            // Mostrar peça já animada em sua posição final
            peca.classList.add('peca-animada');
            const posicoes = {
